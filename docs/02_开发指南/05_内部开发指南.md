...
> **注意**: 如果您是要接入外部应用 (如 Gallery, Toolbox) 的 SSO 登录，请参考 [子系统接入完全手册](./01_子系统接入手册.md)。
...
| `entry.backend` | string | 是 | 后端入口文件路径（默认使用项目根目录相对路径）。 | `apps/demo-app/server/index.js` |
| `entry.frontend` | string | 是 | 前端入口文件路径（默认使用项目根目录相对路径）。 | `apps/demo-app/web/dist/index.html` |
...
> **重要**: CLI 初始化默认生成相对路径，并由主系统按项目根目录解析。仅在运行目录与项目根目录不一致时，才需要改为绝对路径。
...
version: 1.0.0
entry:
  backend: apps/demo-app/server/index.js
  frontend: apps/demo-app/web/dist/index.html
deps: []
envPrefix: DEMO_APP_
meta:
  label: "演示应用"
...
`registerMicroApp` 会为每个子应用自动挂载标准接口：`/<app-name>/health` 与 `/<app-name>/metrics`，无需手动重复实现。
