# Account ↔ Sub-Applications SSO 接入指南 (V2.1)

> **状态**: 已实施 (Implemented)
> **日期**: 2026-02-24
> **适用**: Account (Provider) -> Gallery, Toolbox, LifeOS (Consumers)
...
5.  **回调**: Account 前端将用户重定向回子应用，携带 Hash Fragment。

## 2. 应用注册表 (Application Registry)

V2.1 废弃了静态配置文件，改为 **PostgreSQL 数据库驱动**。

### 2.1 注册新应用 (Visual Method)
**推荐**: 直接在管理员后台进行可视化操作。
...
*   **Q: 登录后跳回 Account 首页而不是子应用**
    *   检查 URL 参数 `target` 是否被正确 URL Encode。
